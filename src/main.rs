...
// get list of R scripts in PipeFish Package
fn get_scripts()-> Vec<String>{
			let a = Command::new("Rscript.exe")
        .args(&["-e", "cat(paste(list.files(system.file('scripts',package='PipeFish')),sep=' '))"])
		.output()
        .expect("...!!!");
...
let mut a = "source(system.file(package='PipeFish',path='scripts/".to_string();
let ref b=input;
let c="'))".to_string();
